...
function registrar(){
	var result = function_ajax({
		...
		'especialista': $("#especialistaagg").val(),
		'estado_visto': $("#estado_vistoagg").val(),
		'estado':'1'
}	,'../controller/chatController.php').then(function(result){
	if(result=="1"){
		alert_success();
		ver_registros();
		$("#chatagg").val("");
...
function cargar_datos(id_chat,chat,usuario,especialista,estado_visto){
	$("#id").val(id_chat);
	$("#chat").val(chat);
	$("#usuario").val(usuario);
	$("#especialista").val(especialista);
	$("#estado_visto").val(estado_visto);
	$("#id_").val(id_);
}
...
function modificar(){
	...
	var estado_visto =  $("#estado_visto").val();
	Swal.fire({
